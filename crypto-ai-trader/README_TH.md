...
เมื่อได้ไฟล์ zip เวอร์ชันใหม่ — เลือกวิธีไหนก็ได้ ข้อมูลเดิมถูกดึงมาให้อัตโนมัติ:

**วิธี A — แตกข้างๆ ของเก่า (ง่ายสุด ไม่ต้องทับ):**
1. แตก zip ไว้ใน**โฟลเดอร์เดียวกับของเก่า** (จะได้โฟลเดอร์ใหม่ เช่น `Aiterra-v1.0.24`)
2. ดับเบิลคลิก **`UPDATE.bat`** ในโฟลเดอร์ใหม่
   → ระบบหาโฟลเดอร์เวอร์ชันเก่าเจอเอง แล้ว**ดึง settings / ประวัติเทรด / โมเดล มาให้**

**วิธี B — แตกทับโฟลเดอร์เดิม:**
1. แตก zip ทับโฟลเดอร์เดิม — เลือก **"Replace the files in the destination"**
2. ดับเบิลคลิก **`UPDATE.bat`**
   → settings.yml เดิมอยู่ครบ และ**เพิ่มเฉพาะ option ใหม่**ของเวอร์ชันนี้เข้าไป (ค่าที่คุณตั้งไว้ชนะเสมอ)

> ทั้งสองวิธี: ก่อนแก้ settings.yml จะ**สำรองไฟล์ `.bak`** ไว้ให้อัตโนมัติ — ไม่มีทางเสียข้อมูล

ข้อมูลที่ถูกเก็บไว้เสมอ (ไม่เคยอยู่ใน zip):
